...
$parsedHost = parse_url($scheme . '://' . $rawHost, PHP_URL_HOST);
$parsedPort = parse_url($scheme . '://' . $rawHost, PHP_URL_PORT);

if (!is_string($parsedHost) || $parsedHost === '') {
  $resolvedHost = preg_replace('/[\/?#].*$/', '', (string)$rawHost);
} else {
  $hostForUrl = $parsedHost;
  if (strpos($hostForUrl, ':') !== false && $hostForUrl[0] !== '[') {
    $hostForUrl = '[' . $hostForUrl . ']';
  }
  $resolvedHost = $parsedPort !== null ? ($hostForUrl . ':' . $parsedPort) : $hostForUrl;
}
...
<body>
  ...
  <script>
    ...
    const scrollPositionInput = document.getElementById('scrollPosition');

    if (flagForm && scrollPositionInput) {
      flagForm.addEventListener('submit', () => {
        const scrollY = window.scrollY || window.pageYOffset || 0;
        scrollPositionInput.value = String(scrollY);
        sessionStorage.setItem('mockctf-home-scroll', String(scrollY));
      });
    }

    window.addEventListener('load', () => {
      const savedScroll = sessionStorage.getItem('mockctf-home-scroll');
      if (savedScroll !== null) {
        window.scrollTo({ top: Number(savedScroll) || 0, behavior: 'auto' });
        sessionStorage.removeItem('mockctf-home-scroll');
      }
    });
